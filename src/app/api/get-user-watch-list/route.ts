...
export async function POST(req: Request, res: Response) {
	...
	try {
		...
		const result = await axios({
			...
			data: JSON.stringify({
				query:
					"query UserWatchList ($username: String!){" +
					"   getUserWatchList(username: $username){" +
					"        id " +
					"        title " +
					"        coverImageUrl " +
					"   }" +
					"}",
				opreationName: "UserWatchList",
		...
		if (result.data.data.getUserWatchList)
			return new NextResponse(
				JSON.stringify(result.data.data.getUserWatchList),
				{
					status: 200,
					headers: {
						"Content-Type": "application/json",
