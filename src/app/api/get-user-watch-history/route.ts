...
export async function POST(req: Request, res: Response) {
	...
	try {
		...
		const result = await axios({
			...
			data: JSON.stringify({
				query:
					"query UserWatchHistory ($username: String!){" +
					"   getUserWatchHistory(username: $username){" +
					"        id " +
					"        title " +
					"        coverImageUrl " +
					"   }" +
					"}",
				opreationName: "UserWatchHistory",
				variables: { username: body.username },
			}),
			headers: {
		...
		if (result.data.data.getUserWatchHistory)
			return new NextResponse(
				JSON.stringify(result.data.data.getUserWatchHistory),
				{
					status: 200,
					headers: {
						"Content-Type": "application/json",
